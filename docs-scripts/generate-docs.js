...
const index = nunjucks.render(path.join('..', 'docs-templates', 'index.njk'), {title: 'Rules', allRuleInfo: allRuleInfo});

fs.writeFile(path.join(outputDirectory, 'index.html'), index, err => {
    if (err) {
      console.error(err)
      return
    }
...
const about = nunjucks.render('../docs-templates/about.njk', {title: 'About'});
fs.writeFile(path.join(outputDirectory, 'about.html'), about, err => {
    if (err) {
      console.error(err)
      return
    }
